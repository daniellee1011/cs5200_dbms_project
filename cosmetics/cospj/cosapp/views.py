        ...
        if check_superuser(request.user.username):
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    storeList = ','.join(str(i)
                                         for i in form.cleaned_data['stores'])
                    connection = connections['default']
                    with connection.cursor() as cursor:
...
def productType(request, id):
    ...
    if form.is_valid():
        print('productType: form.is_valid() = True')
        if form.cleaned_data['category']:
            products = products.filter(
                productType__productCategory__cateName=form.cleaned_data['category'])
        if form.cleaned_data['brand']:
            products = products.filter(
                cosmeticBrand__brandName=form.cleaned_data['brand'])
        ...
        if form.cleaned_data['min_rating']:
            products = products.filter(
                avgRating__gte=form.cleaned_data['min_rating'])
    else:
        print('productType: form.is_vaild() = False')
        print(form.errors)
        print('form.errors the above')
...
def update_profile(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            connection = connections['default']
            with connection.cursor() as cursor:
                cursor.callproc('update_profile', [username, form.cleaned_data['email'], form.cleaned_data['nickname'],
                                form.cleaned_data['age'], form.cleaned_data['gender'], form.cleaned_data['skin_type'], form.cleaned_data['address']])
...
def product_list(request):
    ...
    if form.is_valid():
        ...
        if form.cleaned_data['min_rating']:
            products = products.filter(
                avgRating__gte=form.cleaned_data['min_rating'])
    return render(request, 'productlist.html', {'form': form, 'productlist': products})
